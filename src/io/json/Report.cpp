...
std::expected<void, std::error_code> WriteProductsJson(
    std::span<const Product> products, const fs::path& out_path) {
    if (out_path.empty()) {
        return std::unexpected(
            std::make_error_code(std::errc::invalid_argument));
    }
    return WriteJsonPayload(BuildProductViews(products), out_path);
}

std::expected<void, std::error_code> WriteRecipesJson(
    std::span<const Recipe> recipes, const fs::path& out_path) {
    if (out_path.empty()) {
        return std::unexpected(
            std::make_error_code(std::errc::invalid_argument));
    }
    return WriteJsonPayload(BuildRecipeViews(recipes), out_path);
}
